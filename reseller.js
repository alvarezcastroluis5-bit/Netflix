import {
  ...
  toast, openModal, wireNavigation, setupLogin
} from "./core.js";
import { CREATE_USER_FUNCTION } from "./config.js";
...
function createUserModal() {
  ...
  $("#saveUser", modal.root).addEventListener("click", async () => {
    ...
    const { data, error } = await supabase.functions.invoke(CREATE_USER_FUNCTION, { body });
...
function createAccountModal() {
  const modal = openModal({
    title: "Añadir cuenta Netflix",
    body: `<form id="createAccountForm" class="form-grid">
      <label><span>Correo de la cuenta</span><input name="current_email" type="email" required></label>
      <label><span>Tipo</span><select name="account_type"><option>Cuenta completa</option><option>Pantalla</option></select></label>
      <label class="full"><span>Notas internas</span><textarea name="notes"></textarea></label>
    </form>`,
    actions: `<button class="btn secondary modal-cancel">Cancelar</button><button id="saveAccount" class="btn primary">Guardar</button>`
  });
  $(".modal-cancel", modal.root).addEventListener("click", modal.close);
  $("#saveAccount", modal.root).addEventListener("click", async () => {
    ...
    payload.current_email = payload.current_email.trim().toLowerCase();
    payload.created_by = state.profile.id;
    const { error } = await supabase.from("netflix_accounts").insert(payload);
    if (error) return toast(error.message, "error");
