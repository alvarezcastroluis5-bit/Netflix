-- =========================================================
-- BLOQUE 17 · FLUJO CONTROLADO DE SOPORTE
--
-- OPERADOR:
...
-- ---------------------------------------------------------
-- 1. CREACIÓN: SIEMPRE COMIENZA ABIERTA
-- ---------------------------------------------------------
...
begin
  ...
  where account.service=v_service
    ...
    and (
      private.is_staff()
      or private.can_view_account(account.id)
    )
  limit 1;

  if v_account is null then
    raise exception
      'No se encontró una cuenta visible con ese correo y plataforma.';
  end if;
...
revoke all
on function public.create_support_ticket_v2(
...
begin
  if not private.is_staff() then
    raise exception
      'Solo administración o soporte pueden cambiar el estado.';
  end if;

  if v_status not in('open','in_review','closed') then
  ...
  set
    ...
    assigned_support_id=case
      when v_status='open' then assigned_support_id
      else v_actor
    end,
    closed_at=case
  ...
  return jsonb_build_object(
    ...
    'status',v_status,
    'message',case
      when v_status='in_review' then 'Ticket marcado en proceso.'
      when v_status='closed' then 'Ticket cerrado correctamente.'
...
revoke all
on function public.staff_set_ticket_status_v17(uuid,text)
from public,anon;
...
-- ---------------------------------------------------------
-- 3. RESPUESTAS SEGÚN TIPO DE CASO
...
revoke all
on function public.staff_send_ticket_response_v17(uuid,text,text)
from public,anon;
...
-- ---------------------------------------------------------
-- 4. CAÍDA: CAMBIO DE CUENTA SIN CERRAR AUTOMÁTICAMENTE
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 5. NOTIFICACIÓN AL CERRAR, SIN NÚMERO VISIBLE
-- ---------------------------------------------------------
...
select 'BLOQUE 17 CREADO CORRECTAMENTE' as resultado;
