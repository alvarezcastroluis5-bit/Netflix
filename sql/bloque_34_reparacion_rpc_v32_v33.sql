...
-- ============================================================
-- BLOQUE 34 · REPARACIÓN AUTÓNOMA V6.15.1
-- Reinstala las RPC V32 y V33 utilizadas por la página.
-- ============================================================

-- V32: eliminar firmas anteriores para evitar errores 42P13.
drop function if exists public.staff_list_service_accounts_v32();
drop function if exists public.reseller_list_branch_accounts_v32();
drop function if exists public.staff_list_user_branch_accounts_v32(uuid);
drop function if exists public.reseller_list_user_branch_accounts_v32(uuid);
drop function if exists public.reassign_account_hierarchical_v32(uuid,uuid,date);
drop function if exists public.admin_edit_service_account_v32(
  uuid,text,text,text,uuid,date
);
drop function if exists public.bulk_assign_service_accounts_v32(
  text,text[],uuid,date
);

-- V33: eliminar firmas anteriores para que la reparación sea autónoma.
drop function if exists public.staff_list_tickets_v33();
drop function if exists public.staff_list_ticket_messages_v33(uuid);
drop function if exists public.create_support_ticket_v2(
  text,text,text,text,text
);
drop function if exists public.reseller_add_ticket_message_v33(uuid,text);
drop function if exists public.staff_send_ticket_response_v17(
  uuid,text,text
);
drop function if exists public.reseller_list_change_history_v33();
drop function if exists public.reseller_dashboard_metrics_v33();
drop function if exists public.reassign_account_hierarchical_v33(
  uuid,uuid,date
);
drop function if exists public.admin_edit_service_account_v33(
  uuid,text,text,text,uuid,date
);
drop function if exists public.bulk_assign_service_accounts_v33(
  text,text[],uuid,date
);
...
-- Verificación interna: si falta una función, la transacción genera error.
do $verify$
declare
  v_missing text;
begin
  select string_agg(required.signature, ', ')
  into v_missing
  from (
    values
  ...
  if v_missing is not null then
    raise exception
      'La reparación quedó incompleta. Faltan: %',
      v_missing;
  end if;
end;
$verify$;
...
-- Resultado visible en Supabase.
select
  required.signature as funcion,
...
select
  'BLOQUE 34 CREADO CORRECTAMENTE: FUNCIONES V32 Y V33 INSTALADAS'
  as resultado;
