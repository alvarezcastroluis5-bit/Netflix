console.info("Centro Premium core V6.9.18.1 · loader inteligente cargado");
...
/*
 * V6.9.18.1 · CARGADOR INTELIGENTE
 *
 * - Las actualizaciones automáticas y recurrentes trabajan en silencio.
 * - El cargador global se reserva para acciones iniciadas por el usuario.
 * - La navegación de secciones mantiene su cargador explícito.
 * - Ninguna actualización silenciosa cambia la sección, cierra modales
 *   o interrumpe el formulario que el usuario esté utilizando.
 */
let silentRequestDepth = 0;
let recentUserActionUntil = 0;
let interactiveRequestCount = 0;

const USER_ACTION_WINDOW_MS = 4000;
const USER_ACTION_CHAIN_GRACE_MS = 550;

function markRecentUserAction(duration = USER_ACTION_WINDOW_MS) {
  recentUserActionUntil = Math.max(
    recentUserActionUntil,
    Date.now() + Math.max(500, Number(duration) || USER_ACTION_WINDOW_MS)
  );
}
...
function registerUserAction(event) {
  if (!event?.isTrusted) return;
  if (!isUserActionElement(event.target)) return;
  ...
  markRecentUserAction();
}

document.addEventListener("pointerdown", registerUserAction, true);
document.addEventListener("submit", event => {
  if (event.isTrusted) markRecentUserAction(7000);
}, true);
document.addEventListener("change", registerUserAction, true);
...
async function trackedFetch(input, init) {
   ...
   * - sincronizaciones de fondo.
   *
   * Todas se ejecutan sin bloquear ni desenfocar la interfaz.
   */
  const isSilent =
    silentRequestDepth > 0 ||
    document.visibilityState === "hidden";

  const belongsToUserAction =
    !isSilent &&
    (
      interactiveRequestCount > 0 ||
      Date.now() <= recentUserActionUntil
    );

  if (!belongsToUserAction) {
    return nativeFetch(input, init);
  }

  interactiveRequestCount += 1;
  markRecentUserAction(7000);
  showPageLoader("Procesando tu acción…");

  try {
    return await nativeFetch(input, init);
  } finally {
    interactiveRequestCount = Math.max(0, interactiveRequestCount - 1);
    hidePageLoader();

    if (interactiveRequestCount === 0) {
      recentUserActionUntil = Date.now() + USER_ACTION_CHAIN_GRACE_MS;
    }
  }
}
