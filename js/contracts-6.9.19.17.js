console.info("Centro Premium contratos V6.9.19.21 cargado · logo oficial y sin documento de identidad");
...
const CONTRACT_LOGO_PATH="assets/centro-premium-contract-logo.png";
...
function loadContractLogoDataUrl(){
  if(contractLogoDataUrlPromise)return contractLogoDataUrlPromise;

  contractLogoDataUrlPromise=fetch(CONTRACT_LOGO_PATH,{cache:"force-cache"})
    .then(response=>{
    ...
    .then(blob=>new Promise((resolve,reject)=>{
      ...
      reader.onerror=()=>reject(new Error("No se pudo preparar el logo para el PDF."));
      reader.readAsDataURL(blob);
    }))
    .catch(error=>{
      console.warn("El PDF se generará sin logo porque no se pudo cargar:",error);
      return null;
    });
...
function buildContractPdf({prepared,signatureDataUrl,signedAt,logoDataUrl}){
  ...
  const titleLines=doc.splitTextToSize(
    prepared.template_title||"CONTRATO CENTRO PREMIUM",
    174
  );
  const titleY=logoDataUrl?51:18;
  doc.text(titleLines,pageWidth/2,titleY,{align:"center"});

  const metaY=titleY+(titleLines.length-1)*6+10;
  doc.setFontSize(9);
  doc.setFont("helvetica","normal");
  doc.text(`Contrato: ${prepared.contract_number}`,18,metaY);
  doc.text(`Versión: ${prepared.template_version}`,18,metaY+5);
  doc.text(`Fecha de aceptación: ${signedAt.toLocaleString("es-BO")}`,18,metaY+10);

  const dividerY=metaY+15;
  doc.setDrawColor(190);
  doc.line(18,dividerY,192,dividerY);

  const dataTitleY=dividerY+7;
  doc.setFont("helvetica","bold");
  doc.setFontSize(10);
  doc.text("DATOS DEL DISTRIBUIDOR",18,dataTitleY);
  doc.setFont("helvetica","normal");
  doc.text(`Nombre: ${prepared.signer_full_name}`,18,dataTitleY+7);
  doc.text(`Correo: ${prepared.email}`,18,dataTitleY+13);
  doc.text(`WhatsApp: ${prepared.whatsapp}`,18,dataTitleY+19);
  if(prepared.business_name){
    doc.text(`Nombre comercial: ${prepared.business_name}`,18,dataTitleY+25);
  }

  const bodyStart=prepared.business_name?dataTitleY+33:dataTitleY+27;
  ...
  return new Promise((resolve,reject)=>{
      ...
      <div class="contract-gate" role="dialog" aria-modal="true" aria-labelledby="contractGateTitle">
        <section class="contract-gate-card">
          <header class="contract-gate-header">
            <div class="contract-gate-heading">
              <img class="contract-brand-logo" src="${CONTRACT_LOGO_PATH}" alt="Centro Premium">
              <div class="contract-gate-title-copy">
    ...
    form.addEventListener("submit",async event=>{
      ...
      try{
        ...
        const logoDataUrl=await loadContractLogoDataUrl();
...
function renderContractRows(key){
  ...
  const visible=rows.filter(row=>{
    ...
    return `${row.contract_number||""} ${row.full_name||""} ${row.business_name||""} ${row.email||""}`
      .toLowerCase()
      .includes(query);
  });
  ...
  table.innerHTML=visible.length
    ...
    `).join("")
    :`<tr><td colspan="7"><div class="empty-state">No hay contratos para mostrar.</div></td></tr>`;
...
export async function loadContractCenter(config){
  ...
  const table=$(config.tableSelector);
  if(table){
    table.innerHTML=`<tr><td colspan="7"><div class="empty-state">Cargando contratos...</div></td></tr>`;
  }

  const {data,error}=await supabase.rpc("list_visible_contracts_v50");
  if(error){
    if(table){
      table.innerHTML=`<tr><td colspan="7"><div class="empty-state">${safeText(error.message)}</div></td></tr>`;
    }
