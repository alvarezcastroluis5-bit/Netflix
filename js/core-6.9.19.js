console.info("Centro Premium core V6.9.19.19 cargado");
...
export function setupLogin({allowedRoles,onAuthenticated,beforeEnter=null}) {
  ...
  const enter=async()=>{
    ...
    try{
      ...
      if(typeof beforeEnter==="function"){
        await beforeEnter({session,profile});
      }
...
let persistentNotificationActiveRecipientId="";
const persistentNotificationSuppressedIds=new Set();

const NOTIFICATION_POPUP_DURATION_MS=60*1000;
const NOTIFICATION_REMINDER_INTERVAL_MS=5*60*1000;

function removePersistentNotificationPopup(){
  window.clearTimeout(persistentNotificationHideTimer);
  persistentNotificationHideTimer=null;
  persistentNotificationActiveRecipientId="";
  document.getElementById("persistentNotificationAlert")?.remove();
}
...
function syncNotificationReminder(unread=[]){
  ...
  if(notificationReminderSignature===signature&&notificationReminderTimer){
    return;
  }
  ...
  notificationReminderTimer=window.setInterval(async()=>{
    const pending=(document.__notificationItems||[])
    ...
    /*
     * El recordatorio solo reproduce el sonido. No vuelve a abrir
     * la tarjeta emergente que ya desapareció después de un minuto.
     */
    await playNotificationSound();
...
export function renderPersistentNotificationAlert(
  ...
  /* Libera IDs que ya dejaron de estar pendientes. */
  [...persistentNotificationSuppressedIds].forEach(id=>{
    if(!unreadIds.has(id)){
      persistentNotificationSuppressedIds.delete(id);
    }
  });

  if(!unread.length){
    removePersistentNotificationPopup();
    stopNotificationReminder();
    return;
  }

  /* La campana permanece pendiente y el sonido se repite cada 5 minutos. */
  syncNotificationReminder(unread);

  /*
   * Solo abre una tarjeta para avisos que todavía no fueron ocultados
   * automáticamente en esta sesión. Los avisos ocultados siguen en la
   * campana hasta que el usuario los marque como leídos.
   */
  const item=unread.find(
    row=>!persistentNotificationSuppressedIds.has(row.id)
  );
  ...
  const isNewPopup=
    persistentNotificationActiveRecipientId!==item.id||!root;
  ...
  $(".persistent-read-action",root)?.addEventListener(
    ...
    async()=>{
      ...
      if(marked){
        persistentNotificationSuppressedIds.delete(item.id);

        /*
         * onRead puede haber cargado el siguiente aviso pendiente.
         * No lo cerramos si ya reemplazó al aviso que se acaba de leer.
         */
        if(persistentNotificationActiveRecipientId===item.id){
          removePersistentNotificationPopup();
        }
      }else{
        button.disabled=false;
  ...
  /*
   * Los refrescos automáticos del panel no reinician este minuto.
   * Así el emergente desaparece realmente a los 60 segundos.
   */
  if(isNewPopup){
    persistentNotificationActiveRecipientId=item.id;
    window.clearTimeout(persistentNotificationHideTimer);

    const pendingAtOpen=unread.map(row=>row.id);

    persistentNotificationHideTimer=window.setTimeout(()=>{
      pendingAtOpen.forEach(id=>
        persistentNotificationSuppressedIds.add(id)
      );
