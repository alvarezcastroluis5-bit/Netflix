console.info("Centro Premium admin V6.9.19.21 cargado");
import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,startNotificationWatcher,openNetflixIntegrated,showSection} from "./core-6.9.19.10.js?v=6.9.19.21";
import {COUNTRY_MAP_COUNTRIES,countryMapByCode,countryMapPoint,countryFlagEmoji,whatsappDigits} from "./country-map-data-6.9.19.13.js";
import {
  configureContractModule,
  bindContractCenter,
  loadContractCenter
} from "./contracts-6.9.19.17.js?v=6.9.19.21";

configureContractModule({supabase,$,escapeHtml,formatDate,toast});

const state={profile:null,users:[],accounts:[],assignments:[],tickets:[],history:[],content:[],notifications:[],notificationHistory:[],helpArticles:[],countryDistributors:[],panelSettings:null,services:[]};
const adminAccountPager={page:1,pageSize:25};
const adminTicketPager={page:1,pageSize:25};
let adminTicketView="";
let adminNotificationSending=false;

function adminContractConfig(){
  return {
    key:"admin-contracts",
    profile:state.profile,
    tableSelector:"#adminContractsTable",
    searchSelector:"#adminContractSearch",
    countSelector:"#adminContractsCount",
    refreshSelector:"#refreshAdminContracts"
  };
}
...
setupLogin({
  ...
  onAuthenticated:async({profile})=>{
    ...
    if(profile.role==="support"){
      ...
      ["services","users","content","help","notifications","contracts","panelsettings"].forEach(section=>{
...
function bindActions(){
  bindContractCenter(adminContractConfig());
  ...
  $("#openCountryMapManager")?.addEventListener("click",()=>countryMapManagerModal(null));
...
async function loadAll(){
  ...
  if(state.profile.role==="admin"){
    commonTasks.push(loadContent(),loadHelpArticles());
  }

  /* Admin y soporte reciben avisos con la misma lógica. */
  commonTasks.push(loadAdminNotifications());
...
async function loadSection(section){
  ...
  if(section==="contracts"&&state.profile.role==="admin")await loadContractCenter(adminContractConfig());
...
function renderHelpAdmin(){
    ...
    return `
      <article class="help-admin-card" data-help-card="${article.id}">
        ...
        <div class="help-admin-media-summary">
          ...
          ${article.show_country_map?`<span class="status-pill cyan">Mapa público configurado</span>`:""}
        </div>
        <div class="help-admin-actions">
          ${article.show_country_map?`<button type="button" class="action-button cyan" data-help-country-map="${article.id}">🌍 Gestionar mapa</button><a class="action-button help-public-map-link" href="./distribuidores-por-pais.html" target="_blank" rel="noopener noreferrer">↗ Ver web pública</a>`:""}
  ...
  $$('[data-help-country-map]').forEach(button=>{
    button.onclick=()=>countryMapManagerModal(button.dataset.helpCountryMap);
  });
...
async function loadCountryDistributorsAdmin(){
  const {data,error}=await supabase
  ...
  return state.countryDistributors;
}

function countryMapMarkerMarkup(row,{selectedCode="",admin=false}={}){
  ...
  const selected=String(row.country_code)===String(selectedCode);
  const active=row.is_active!==false;

  return `
    <button
      type="button"
      class="country-map-pin ${active?"is-active":"is-inactive"} ${selected?"is-selected":""} ${admin?"is-admin":""}"
      style="left:${point.left};top:${point.top}"
      data-country-map-code="${escapeHtml(row.country_code)}"
      aria-label="${escapeHtml(country.name||row.country_name)}: ${escapeHtml(row.distributor_name||"Sin distribuidor")}"
...
function adminCountryMapMarkup(rows,selectedCode=""){
  return `
    <div class="country-map-stage country-map-stage-admin">
      <img src="./assets/world-map-public.svg?v=6.9.19.18" alt="Mapa mundial con todos los países">
      <div class="country-map-pin-layer">
        ${rows.map(row=>countryMapMarkerMarkup(row,{selectedCode,admin:true})).join("")}
      </div>
    </div>
  `;
}

async function countryMapManagerModal(articleId){
  try{
    await loadCountryDistributorsAdmin();
  }catch(error){
    toast(error.message,"error");
    return;
  }

  let selectedCode=state.countryDistributors[0]?.country_code||"BO";
  const countryOptions=COUNTRY_MAP_COUNTRIES.map(country=>`
    <option value="${escapeHtml(country.code)}">${escapeHtml(country.name)}</option>
  `).join("");

  const modal=openModal({
    title:"Distribuidores por país",
    extraWide:true,
    body:`
      <div class="notice-box">
        Configura aquí el distribuidor oficial de cada país. El mapa ya no aparece dentro del panel de distribuidores: se publica en una página independiente, únicamente para consulta.
      </div>
      <div class="country-map-public-url-box">
        <div>
          <small>ENLACE PÚBLICO DEL MAPA</small>
          <strong id="countryPublicMapUrl"></strong>
        </div>
        <div class="country-map-public-url-actions">
          <button id="copyCountryPublicMapUrl" type="button" class="btn secondary">Copiar enlace</button>
          <a class="btn primary" href="./distribuidores-por-pais.html" target="_blank" rel="noopener noreferrer">Abrir web pública</a>
        </div>
      </div>
      <div class="country-map-admin-layout">
        <section class="country-map-admin-preview">
          <div id="adminCountryMapCanvas"></div>
          <div class="country-map-legend">
            <span><i class="active"></i> País activo</span>
            <span><i class="inactive"></i> País oculto</span>
          </div>
        </section>
        <section class="country-map-admin-editor">
          <form id="countryDistributorForm" class="form-grid">
            <label class="full"><span>País</span><select name="country_code" required>${countryOptions}</select></label>
            <label class="full"><span>Nombre comercial del distribuidor</span><input name="distributor_name" minlength="2" required placeholder="Ej.: Centro Premium Bolivia"></label>
            <label class="full"><span>WhatsApp del distribuidor</span><input name="whatsapp" inputmode="tel" required placeholder="Ej.: +59170000000"><small class="field-help">Incluye el código del país. El botón de contacto utilizará este número.</small></label>
            <label class="inline-check full"><input name="is_active" type="checkbox" checked> Mostrar este país y su pin a los usuarios</label>
          </form>
          <div class="country-map-editor-actions">
            <button id="saveCountryDistributor" type="button" class="btn primary">Guardar país</button>
            <button id="deleteCountryDistributor" type="button" class="btn danger">Eliminar registro</button>
          </div>
          <div class="country-map-configured-list" id="countryConfiguredList"></div>
        </section>
      </div>
    `,
    actions:`<button class="btn secondary modal-cancel">Cerrar</button>`
  });

  const form=$("#countryDistributorForm",modal.root);
  const mapRoot=$("#adminCountryMapCanvas",modal.root);
  const listRoot=$("#countryConfiguredList",modal.root);
  const deleteButton=$("#deleteCountryDistributor",modal.root);
  const publicMapUrl=new URL("./distribuidores-por-pais.html",window.location.href).href;
  const publicMapUrlLabel=$("#countryPublicMapUrl",modal.root);
  const copyPublicMapButton=$("#copyCountryPublicMapUrl",modal.root);

  if(publicMapUrlLabel)publicMapUrlLabel.textContent=publicMapUrl;
  if(copyPublicMapButton){
    copyPublicMapButton.onclick=async()=>{
      try{
        await navigator.clipboard.writeText(publicMapUrl);
        toast("Enlace público copiado.");
      }catch{
        window.prompt("Copia el enlace público del mapa:",publicMapUrl);
      }
    };
  }

  const findRecord=code=>state.countryDistributors.find(row=>row.country_code===code)||null;

  const renderConfiguredList=()=>{
    listRoot.innerHTML=state.countryDistributors.length
      ?`<strong>Países configurados (${state.countryDistributors.length})</strong><div class="country-map-configured-scroll">${state.countryDistributors.map(row=>`
        <button type="button" class="country-map-configured-item ${row.country_code===selectedCode?"active":""}" data-country-list-code="${escapeHtml(row.country_code)}">
          <span>${countryFlagEmoji(row.country_code)}</span>
          <span><b>${escapeHtml(row.country_name)}</b><small>${escapeHtml(row.distributor_name)} · ${row.is_active?"Activo":"Oculto"}</small></span>
        </button>
      `).join("")}</div>`
      :`<div class="empty-state">Todavía no se registraron distribuidores por país.</div>`;

    $$('[data-country-list-code]',listRoot).forEach(button=>{
      button.onclick=()=>selectCountry(button.dataset.countryListCode);
    });
  };

  const renderMap=()=>{
    mapRoot.innerHTML=adminCountryMapMarkup(state.countryDistributors,selectedCode);
    $$('[data-country-map-code]',mapRoot).forEach(button=>{
      button.onclick=()=>selectCountry(button.dataset.countryMapCode);
    });
  };

  const fillForm=()=>{
    const country=countryMapByCode(selectedCode)||COUNTRY_MAP_COUNTRIES[0];
    const row=findRecord(selectedCode);
    form.elements.country_code.value=country.code;
    form.elements.distributor_name.value=row?.distributor_name||"";
    form.elements.whatsapp.value=row?.whatsapp||"";
    form.elements.is_active.checked=row?row.is_active!==false:true;
    deleteButton.disabled=!row;
  };

  const selectCountry=code=>{
    selectedCode=String(code||"BO").toUpperCase();
    fillForm();
    renderMap();
    renderConfiguredList();
  };

  form.elements.country_code.onchange=event=>selectCountry(event.target.value);
  $(".modal-cancel",modal.root).onclick=modal.close;

  $("#saveCountryDistributor",modal.root).onclick=async()=>{
    if(!form.reportValidity())return;
    const country=countryMapByCode(form.elements.country_code.value);
    if(!country){
      toast("Selecciona un país válido.","error");
      return;
    }

    const digits=whatsappDigits(form.elements.whatsapp.value);
    if(digits.length<7||digits.length>16){
      toast("Coloca un número de WhatsApp válido con código de país.","error");
      return;
    }

    const button=$("#saveCountryDistributor",modal.root);
    button.disabled=true;
    button.textContent="Guardando...";

    try{
      const {data,error}=await supabase.rpc("admin_upsert_country_distributor_v49",{
        p_country_code:country.code,
        p_country_name:country.name,
        p_latitude:country.lat,
        p_longitude:country.lng,
        p_distributor_name:form.elements.distributor_name.value,
        p_whatsapp:form.elements.whatsapp.value,
      ...
      renderMap();
      renderConfiguredList();
      toast(data?.message||"Distribuidor por país guardado.");
    }catch(error){
      toast(error.message,"error");
    }finally{
      button.disabled=false;
      button.textContent="Guardar país";
    }
  };
  ...
  deleteButton.onclick=async()=>{
    const row=findRecord(selectedCode);
    if(!row)return;
    if(!confirm(`¿Eliminar el distribuidor configurado para ${row.country_name}?`))return;

    deleteButton.disabled=true;
    try{
      const {data,error}=await supabase.rpc("admin_delete_country_distributor_v49",{
      ...
      renderMap();
      renderConfiguredList();
      toast(data?.message||"Registro eliminado.");
    }catch(error){
      toast(error.message,"error");
    }finally{
      deleteButton.disabled=!findRecord(selectedCode);
    }
  };
...
function helpArticleModal(existing=null){
  ...
  const modal=openModal({
    ...
    body:`
      <form id="helpArticleForm" class="form-grid">
        ...
        <label class="inline-check full help-country-map-toggle"><input name="show_country_map" type="checkbox" ${existing?.show_country_map?"checked":""}> Mostrar debajo de esta publicación el mapa mundial interactivo de distribuidores por país.</label>
  ...
  $("#saveHelpArticle",modal.root).onclick=async()=>{
    ...
    try{
      ...
      const {data,error}=await supabase.rpc("admin_save_help_article_v49",{
        ...
        p_show_country_map:Boolean(form.elements.show_country_map?.checked),
