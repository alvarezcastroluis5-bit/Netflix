import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,openNetflixIntegrated,showSection} from "./core.js?v=5.4";
...
function renderTickets(){
  ...
  const rows=state.tickets.filter(ticket=>{
    ...
    const matchesService=!service||ticket.service===service;
    const matchesStatus=!status||ticket.status===status;

    return matchesText&&matchesService&&matchesStatus;
  });

  $("#ticketsTable").innerHTML=rows.length
    ?rows.map(ticket=>`
      <tr>
        <td>${escapeHtml(ticket.creator?.full_name||"—")}</td>
        <td>${serviceBadge(ticket.service)}</td>
        <td><strong>${escapeHtml(ticket.title)}</strong></td>
        <td>${escapeHtml(ticket.category)}</td>
        <td>${escapeHtml(ticket.reported_email||ticket.account_email_snapshot)}</td>
        <td>
        ...
        <td>
          <button class="round-action" data-ticket-open="${ticket.id}">
            Ver
          </button>
        </td>
      </tr>
...
function openFailureAccountReplacementModal(ticket){
  const modal=openModal({
    ...
    body:`
      <div class="notice-box">
        Esta acción aplicará una garantía aunque el ticket haya sido
  ...
  $("#confirmFailureReplacement",modal.root).onclick=async()=>{
    ...
    try{
      ...
      toast(
        data.message||
        "Cuenta de garantía asignada correctamente."
      );
