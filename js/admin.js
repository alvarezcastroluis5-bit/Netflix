import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,openNetflixIntegrated,showSection} from "./core.js?v=6.12";
...
async function loadAccounts(){
  const {data,error}=await supabase.rpc(
    "staff_list_service_accounts_v28"
  );
...
function adminFullEditAccountModal(id){
  ...
  $("#saveAccountChanges",modal.root).onclick=async()=>{
    ...
    try{
      const {data,error}=await supabase.rpc(
        "admin_edit_service_account_v30",
        {
          p_account_id:id,
          p_service:values.service,
          p_account_type:values.account_type,
