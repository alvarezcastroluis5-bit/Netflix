import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,openNetflixIntegrated,showSection} from "./core.js?v=6.9.1";

const state={profile:null,users:[],accounts:[],assignments:[],tickets:[],history:[],content:[],notifications:[],helpArticles:[],panelSettings:null,services:[]};
  ...
  $("#usersTable").innerHTML=rows.length
    ?rows.map(user=>{
      ...
      const actions=isDistributor
        ?`
          <button class="action-button blue" data-user-accounts="${user.id}">
            Cuentas
          </button>
          <button class="action-button cyan" data-user-assign="${user.id}">
            Asignar
          </button>
          <button class="action-button yellow" data-user-edit="${user.id}">
            Editar
          </button>
          <button class="action-button red" data-user-delete="${user.id}">
            Eliminar
          </button>
        `
        :`
...
function renderPanelServiceManager(){
  ...
  root.innerHTML=state.services.map(service=>`
    <article class="service-manager-card">
      <div class="service-manager-logo">${serviceVisual(service)}</div>
      <div>
        <strong>${escapeHtml(service.name)}</strong>
        <small>${escapeHtml(service.mode)} · ${
  ...
  const modal=openModal({
    ...
    body:`
      <form id="panelServiceForm" class="form-grid">
        ...
        <label><span>URL del logotipo</span>
          <input name="logo_url" type="url" value="${escapeHtml(service?.logo_url||"")}">
        </label>
        ...
        <label><span>Comportamiento</span>
          <select name="mode">
            ...
            <option value="netflix_internal" ${service?.mode==="netflix_internal"?"selected":""}>Netflix interno</option>
          </select>
        </label>
