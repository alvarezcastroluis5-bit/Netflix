import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,openNetflixIntegrated,showSection} from "./core.js?v=6.1";
...
function renderAccounts(){
  ...
  $("#accountsTable").innerHTML=visibleRows.length
    ?visibleRows.map(account=>{
      const assignment=activeAssignmentFor(account.id);
  ...
  const modal=openModal({
    ...
    body:`
      <div class="notice-box">
        ...
        La duración se mantiene en 30 días.
      </div>
  ...
  $("#confirmAdminBulkUpdateDates",modal.root).onclick=async()=>{
    ...
    try{
      const {data,error}=await supabase.rpc(
        "staff_bulk_update_account_dates_v23",
        {
      ...
      toast(
        ...
        `${data.not_assigned||0} sin asignación activa y `+
