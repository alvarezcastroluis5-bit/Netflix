import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,wireNavigation,setupLogin,callUserManager,parseEmailBlock,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,openNetflixIntegrated,showSection} from "./core.js?v=5.0";

const state={profile:null,users:[],accounts:[],assignments:[],tickets:[],history:[],content:[],notifications:[],helpArticles:[]};
...
setupLogin({
  ...
  onAuthenticated:async({profile})=>{
    ...
    if(profile.role==="support"){
      ...
      $("#adminAccountBulkActions")?.remove();
...
function renderAccounts(){
  ...
  const rows=state.accounts.filter(account=>
    `${account.current_email} ${ownerDisplay(account)}`
      .toLowerCase()
      .includes(query)
  ...
  $("#accountsTable").innerHTML=rows.length
    ?rows.map(account=>{
      ...
      const actions=state.profile.role==="admin"
        ?`
          <div class="action-group account-admin-actions">
            <button
      ...
      return `
        <tr>
          ...
          <td>${formatDate(assignment?.starts_on||account.created_at)}</td>
          ...
          <td>
            <span class="status-pill ${statusTone(account.status)}">
  ...
  if(state.profile.role==="support"){
    ...
    $("#supportResolvedList").innerHTML=resolved.slice(0,6).map(ticket=>`
      <button class="stack-row" data-ticket-open="${ticket.id}">
        <div>
          <strong>${escapeHtml(ticket.title)}</strong>
          <small>#${ticket.ticket_number} · ${escapeHtml(ticket.creator?.full_name||"Usuario")}</small>
        </div>
...
function adminFullEditAccountModal(id){
  ...
  $("#saveAccountChanges",modal.root).onclick=async()=>{
    ...
    try{
      const {data,error}=await supabase.rpc(
        "admin_edit_service_account",
        {
          p_account_id:id,
          p_service:values.service,
          p_email:account.current_email,
          p_account_type:values.account_type,
...
function createStaffTicketModal(){
  const modal=openModal({
    ...
    body:`
      ...
      <form id="staffTicketForm" class="form-grid">
        ...
        <label><span>Categoría</span><select name="category"><option>Caída</option><option>Falla</option><option>Contraseña incorrecta</option></select></label>
        <label class="full"><span>Correo de la cuenta</span><input name="reported_email" type="email" list="staffTicketAccounts" required><datalist id="staffTicketAccounts"></datalist></label>
  ...
  const updateAccounts=()=>{
    $("#staffTicketAccounts",modal.root).innerHTML=state.accounts
      .filter(account=>account.service===form.elements.service.value)
      .map(account=>`<option value="${escapeHtml(account.current_email)}">${escapeHtml(ownerDisplay(account))}</option>`)
      .join("");
  };

  form.elements.service.onchange=updateAccounts;
  updateAccounts();
  $(".modal-cancel",modal.root).onclick=modal.close;
