console.info("Centro Premium reseller V6.9.19 cargado");
...
const state={profile:null,network:[],accounts:[],assignments:[],tickets:[],history:[],content:[],notifications:[],helpArticles:[],parent:null,metrics:null,panelSettings:null,services:[]};
...
function scheduleSilentSync(key,task,{showLoaderWhenActive=false,activeSection=""}={}){
  ...
  silentPanelSyncTimers.set(key,window.setTimeout(async()=>{
    ...
    }catch(error){console.warn(`Sincronización silenciosa ${key}:`,error);}
  },250));
}
function startSilentPanelSync(userId){
  if(silentPanelSyncChannel)supabase.removeChannel(silentPanelSyncChannel);
  silentPanelSyncChannel=supabase.channel(`panel-sync-${userId}-${crypto.randomUUID()}`)
    .on("postgres_changes",{event:"*",schema:"public",table:"service_catalog"},()=>scheduleSilentSync("services",loadServiceCatalog,{showLoaderWhenActive:true,activeSection:"services"}))
    .on("postgres_changes",{event:"*",schema:"public",table:"panel_settings"},()=>scheduleSilentSync("brand",loadPanelSettings))
    .on("postgres_changes",{event:"*",schema:"public",table:"entertainment_content"},()=>scheduleSilentSync("content",loadContent,{showLoaderWhenActive:true,activeSection:"dashboard"}))
    .on("postgres_changes",{event:"*",schema:"public",table:"account_assignments",filter:`buyer_reseller_id=eq.${userId}`},()=>scheduleSilentSync("accounts",loadAccounts,{showLoaderWhenActive:true,activeSection:"accounts"}))
    .subscribe();
  // Respaldo: sincroniza cambios generales sin interrumpir ni mostrar cargador.
  window.setInterval(()=>{
    if(document.visibilityState!=="visible")return;
    scheduleSilentSync("services-poll",loadServiceCatalog,{showLoaderWhenActive:false});
    scheduleSilentSync("brand-poll",loadPanelSettings,{showLoaderWhenActive:false});
  },60000);
}
