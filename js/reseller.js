    ...
    body:`
      <div class="password-recovery-intro">
        ...
        <div>
          ...
          <p>
            Coloca tu correo de acceso y el WhatsApp real registrado
            de tu superior.
          </p>
        </div>
      </div>
      ...
      <form id="passwordRecoveryValidationForm" class="form-grid">
        ...
        <label class="full">
          ...
          <input
            ...
            inputmode="numeric"
            pattern="[0-9]{8,15}"
            placeholder="591000000"
            required
          >
          <small class="field-help">
            Ejemplo: 591000000. Debes colocar el WhatsApp real
            registrado de tu superior, sin el signo +, espacios ni guiones.
          </small>
        </label>
      </form>
    `,
...
async function loadAll(){
  await Promise.all([
    runStartupTask("Red",()=>loadNetwork()),
    runStartupTask("Cuentas",()=>loadAccounts()),
    runStartupTask("Tickets",()=>loadTickets()),
    runStartupTask(
      "Métricas",
      ()=>loadDashboardMetrics()
    ),
    runStartupTask("Historial",()=>loadHistory()),
    runStartupTask("Contenido",()=>loadContent()),
    runStartupTask(
      "Preguntas y uso",
      ()=>loadHelpArticles()
    ),
    runStartupTask("Superior",()=>loadParent()),
    runStartupTask(
      "Notificaciones",
      ()=>loadMyNotifications()
    )
    ...
    body:`
      <div class="netflix-account-validation">
        ...
        <div>
          ...
          <h3>Confirma que la cuenta está en tu red</h3>
          <p>
            Puedes validar cuentas tuyas o de cualquiera de tus subordinados. Después se abrirá JP Streaming automáticamente.
          </p>
        </div>
      </div>
      ...
      <form id="netflixAccountValidationForm" class="form-grid">
        ...
        <div class="notice-box full">
          Después de validar verás el país de la cuenta.
          Para restablecer la contraseña, utiliza un VPN de ese país.
        </div>
      </form>
    `,
    actions:`
  ...
  const validate=async()=>{
    ...
    try{
      ...
      openNetflixIntegrated({
        ...
        vpnMessage:data.vpn_message,
        ownerName:data.owner_name||"Sin propietario",
        ownerParentName:
          data.owner_parent_name||"Sin superior",
        accessScope:data.access_scope||"own"
      });
    }catch(error){
      toast(
        error.message||
        "No se pudo validar esta cuenta.",
        "error"
      );
      ...
      button.disabled=false;
      button.textContent="Validar e ingresar";
    }
  };
...
function networkHierarchyPath(user){
  const map=new Map(
    state.network.map(item=>[item.id,item])
  );
  ...
  while(
    ...
    names.unshift(networkDisplayName(current));
...
function networkHierarchyDepth(user){
  return Math.max(0,networkHierarchyPath(user).length-1);
}

function hierarchicalNetwork(rows){
  return [...rows].sort((a,b)=>
    networkHierarchyPath(a)
      .join(" / ")
      .localeCompare(
        networkHierarchyPath(b).join(" / "),
        "es"
      )
  );
}
...
function filteredNetworkUsers(){
  ...
  const rows=state.network.filter(user=>
    user.status==="active"
  );
  ...
  return hierarchicalNetwork(filtered);
}
...
function renderNetwork(){
  ...
  $("#networkTable").innerHTML=rows.length
    ?rows.map(user=>{
      ...
      const depth=networkHierarchyDepth(user);
...
async function loadAccounts(){
  const {data,error}=await supabase.rpc(
    "reseller_list_branch_accounts_v32"
  );
...
function renderAccounts(){
    ...
    ?visibleRows.map(account=>{
      ...
      const action=`
        <div class="action-group">
          <button
            class="action-button yellow"
            data-edit-branch-account="${account.id}"
          >
            Editar
          </button>
        </div>
  ...
  $$("[data-edit-branch-account]").forEach(button=>{
    button.onclick=()=>editBranchAccountModal(
      button.dataset.editBranchAccount
    );
  });
}
...
async function loadHistory(){
  const {data,error}=await supabase.rpc(
    "reseller_list_change_history_v33"
  );
  ...
  if(error){
    ...
    `;
    toast(`No se pudo cargar el historial: ${error.message}`,"error");
    return false;
  }
  ...
  state.history=(data||[]).map(item=>({
    ...item,
    operator:{
      full_name:item.operator_full_name,
      business_name:item.operator_business_name
    }
  }));

  $("#historyTable").innerHTML=state.history.length
    ?state.history.map(history=>`
      <tr>
        <td>${serviceBadge(history.service)}</td>
        <td>
          <div class="change-old">
            Anterior: ${escapeHtml(history.old_email)}
          </div>
          <div class="change-new">
            Nueva: ${escapeHtml(history.new_email)}
          </div>
        </td>
        <td>
          <span class="status-pill orange">
            ${escapeHtml(history.change_type)}
          </span>
        </td>
        <td>
          ${escapeHtml(
            history.operator?.business_name||
            history.operator?.full_name||
            "Sistema"
          )}
        </td>
        <td>${formatDate(history.created_at,true)}</td>
      </tr>
    `).join("")
    :`<tr>
        <td colspan="5" class="empty-cell">
          No existen cambios visibles en tu rama.
        </td>
      </tr>`;
...
async function loadDashboardMetrics(){
  const {data,error}=await supabase.rpc(
    "reseller_dashboard_metrics_v33"
  );
...
function renderDashboard(){
  ...
  const branchAccounts=Number(
    metrics.branch_accounts??state.accounts.length
  );

  const networkDistributors=Number(
    metrics.network_distributors??state.network.length
  );

  const expiring=state.assignments.filter(
    assignment=>
      assignment.status==="active"
      &&Number.isInteger(assignment.days_remaining)
  ...
  $("#statNetwork").textContent=networkDistributors;
  $("#statAccounts").textContent=branchAccounts;
  ...
  $("#dashboardAccountsTable").innerHTML=
    ...
    ||`<tr>
        <td colspan="6" class="empty-cell">
          No existen cuentas visibles en tu rama.
        </td>
      </tr>`;
...
function branchOwnerLabel(user){
  ...
  return `${
    networkHierarchyPath(user).join(" → ")
  } · ${user.email}`;
}

function editBranchAccountModal(id,onSaved=null){
  const account=state.accounts.find(item=>item.id===id);
  if(!account)return;

  const assignment=assignmentFor(account);
  const owners=branchOwnerOptions();

  const modal=openModal({
    title:"Editar cuenta",
    body:`
      <div class="notice-box">
        Puedes cambiar el propietario dentro de tu propia jerarquía.
        El país y el tipo solo pueden modificarse desde administración.
        La fecha es opcional.
      </div>

      <form id="editBranchAccountForm" class="form-grid">
        <label class="full">
          <span>Correo</span>
          <input
          ...
          <select name="owner_id" required>
            ${owners.map(user=>`
              <option value="${user.id}">
                ${escapeHtml(branchOwnerLabel(user))}
              </option>
            `).join("")}
          </select>
          <small class="field-help">
            Aparecen tú y todos los distribuidores de tu rama.
          </small>
        </label>
        ...
        <label>
          ...
          >
          <small class="field-help">
            Vacío deja fecha de corte y días restantes como “—”.
          </small>
        </label>
    ...
    actions:`
      <button class="btn secondary modal-cancel">Cancelar</button>
      <button id="saveBranchAccount" class="btn primary">
        Guardar
      </button>
    `
  });

  const form=$("#editBranchAccountForm",modal.root);
  ...
  $("#saveBranchAccount",modal.root).onclick=async()=>{
    if(!form.reportValidity())return;
    ...
    const button=$("#saveBranchAccount",modal.root);
    button.disabled=true;
    button.textContent="Guardando...";

    try{
      const {data,error}=await supabase.rpc(
        "reassign_account_hierarchical_v33",
        {
          p_account_id:id,
          p_new_owner_id:values.owner_id,
...
async function showNetworkUserAccounts(id){
  ...
  try{
    const {data,error}=await supabase.rpc(
      "reseller_list_user_branch_accounts_v32",
      {p_distributor_id:id}
    );
    ...
    $$("[data-network-account-edit]",modal.root).forEach(button=>{
      button.onclick=()=>{
        const accountId=button.dataset.networkAccountEdit;

        editBranchAccountModal(
          accountId,
...
async function openTicket(id){
  ...
  $("#ticketReplyForm",modal.root)?.addEventListener(
    ...
    async event=>{
      ...
      try{
        const {data,error}=await supabase.rpc(
          "reseller_add_ticket_message_v33",
          {
            p_ticket_id:id,
            p_message:message
          }
