import {supabase,$,$$,escapeHtml,formatDate,statusLabel,statusTone,serviceLabel,serviceBadge,toast,openModal,showSection,wireNavigation,setupLogin,callUserManager,callPublicUserManager,uploadPublicImage,loadNotifications,updateNotificationBadge,showNotificationsModal,renderPersistentNotificationAlert,maybeShowBrowserNotifications,openNetflixIntegrated} from "./core.js?v=5.7";
const state={profile:null,network:[],accounts:[],assignments:[],tickets:[],history:[],content:[],notifications:[],helpArticles:[],parent:null};
...
async function loadAll(){await Promise.allSettled([loadNetwork(),loadAccounts(),loadTickets(),loadHistory(),loadContent(),loadHelpArticles(),loadParent(),loadMyNotifications()]);renderDashboard();}
...
function renderTickets(){
  $("#ticketsTable").innerHTML=state.tickets.length
    ?state.tickets.map(ticket=>`
      <tr>
        ...
        <td>
          <button class="round-action" data-ticket="${ticket.id}">
            Ver
          </button>
        </td>
      </tr>
  ...
  $$("[data-ticket]").forEach(button=>{
    button.onclick=()=>openTicket(button.dataset.ticket);
  });
}
