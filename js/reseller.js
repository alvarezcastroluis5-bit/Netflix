...
function exportResellerAccountsToExcel(){
  const ownAccounts=state.accounts.filter(
    account=>account.current_reseller_id===state.profile.id
  );

  const rows=ownAccounts.map(account=>{
    const assignment=assignmentFor(account);
  ...
  if(!rows.length){
    toast(
      "No tienes cuentas asignadas directamente para exportar.",
      "error"
    );
    return;
  }
  ...
  exportResellerWorkbook(
    rows,
    `mis-cuentas-${new Date().toISOString().slice(0,10)}.xlsx`
  );
