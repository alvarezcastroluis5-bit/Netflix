console.info("Centro Premium core V6.9.15 cargado");
...
export function openNetflixIntegrated() {
  ...
  /*
   * La dirección se arma en ejecución para que no aparezca como texto
   * visible dentro de la interfaz. La herramienta permanece incrustada
   * en el mismo modal y nunca se abre una pestaña nueva.
   */
  ...
  const modal = openModal({
    ...
    body: `
      <section class="netflix-direct-shell">
        <section id="netflixDirectValidationStage">
          <div class="netflix-direct-intro">
            <span class="eyebrow">CONTROL DE ACCESO</span>
            <h3>Validar cuenta Netflix</h3>
            <p>
              Solo podrá ingresar quien tenga la cuenta asignada o sea un
              superior autorizado dentro de la misma jerarquía.
            </p>
          </div>

          <form id="netflixDirectForm" class="netflix-native-form">
            <label>
        ...
        <section
          id="netflixDirectToolStage"
          class="netflix-direct-tool-stage"
          hidden
        >
          <section class="netflix-direct-access-card">
            <div class="netflix-direct-access-copy">
              <span class="eyebrow">PIN DE ACCESO</span>
              <strong class="netflix-direct-pin">${NETFLIX_GENERAL_PIN}</strong>
              <p>
                Coloca el PIN dentro de la herramienta. Después selecciona
                Netflix, pega el correo validado y pulsa Buscar código.
              </p>
            </div>

            <div class="netflix-direct-access-actions">
              <button
                id="copyNetflixDirectPin"
          ...
          <div class="netflix-direct-account-strip">
            <span>Cuenta autorizada</span>
            <strong id="netflixDirectAuthorizedEmail"></strong>
            <small>
              Propietario:
              <span id="netflixDirectOwner"></span>
              · País:
              <span id="netflixDirectCountry"></span>
            </small>
          </div>

          <div class="netflix-direct-stage-tabs" role="group">
            <button
              class="netflix-direct-stage-tab active"
              data-netflix-embed-view="pin"
              type="button"
            >
              1. PIN
            </button>
            <button
              class="netflix-direct-stage-tab"
              data-netflix-embed-view="services"
              type="button"
            >
          ...
          <div
            id="netflixDirectFrameShell"
            class="netflix-direct-frame-shell"
            data-view="pin"
          >
            <div id="netflixDirectLoader" class="netflix-frame-loader">
              <span class="spinner"></span>
              <strong>Cargando herramienta Netflix…</strong>
            </div>

            <iframe
              id="netflixDirectFrame"
            ...
            <div class="netflix-direct-mask netflix-direct-mask-top"></div>
            <div class="netflix-direct-mask netflix-direct-mask-bottom"></div>
            <div class="netflix-direct-mask netflix-direct-mask-help"></div>

            <div
              id="netflixDirectEmailMask"
              class="netflix-direct-email-mask"
              hidden
            >
              <span>Correo electrónico de Netflix</span>
              <strong id="netflixDirectMaskedEmail"></strong>
            </div>
          </div>

          <div class="netflix-direct-guide">
            <p id="netflixDirectGuideText">
              Coloca el PIN 071726 y presiona Continuar dentro de la herramienta.
            </p>
            <div>
              <button
                id="netflixDirectPreviousStage"
                class="btn secondary"
                type="button"
                hidden
              >
  ...
  const validationStage = $("#netflixDirectValidationStage", modal.root);
  const toolStage = $("#netflixDirectToolStage", modal.root);
  const form = $("#netflixDirectForm", modal.root);
  const emailInput = form.elements.email;
  const validateButton = $("#validateNetflixDirect", modal.root);
  const validationBox = $("#netflixDirectValidation", modal.root);
  const authorizedEmail = $("#netflixDirectAuthorizedEmail", modal.root);
  const ownerLabel = $("#netflixDirectOwner", modal.root);
  const countryLabel = $("#netflixDirectCountry", modal.root);
  const maskedEmail = $("#netflixDirectMaskedEmail", modal.root);
  const emailMask = $("#netflixDirectEmailMask", modal.root);
  const copyPinButton = $("#copyNetflixDirectPin", modal.root);
  const copyEmailButton = $("#copyNetflixDirectEmail", modal.root);
  const changeEmailButton = $("#changeNetflixDirectEmail", modal.root);
  const frameShell = $("#netflixDirectFrameShell", modal.root);
  const frame = $("#netflixDirectFrame", modal.root);
  const loader = $("#netflixDirectLoader", modal.root);
  const guideText = $("#netflixDirectGuideText", modal.root);
  const previousStageButton = $("#netflixDirectPreviousStage", modal.root);
  const nextStageButton = $("#netflixDirectNextStage", modal.root);
  const stageTabs = $$("[data-netflix-embed-view]", modal.root);

  const views = ["pin", "services", "email"];
  let verified = null;
  ...
  function setEmbedView(view) {
    const safeView = views.includes(view) ? view : "pin";
    currentViewIndex = views.indexOf(safeView);
    frameShell.dataset.view = safeView;

    stageTabs.forEach(button => {
      button.classList.toggle(
        "active",
        button.dataset.netflixEmbedView === safeView
      );
    });

    previousStageButton.hidden = currentViewIndex === 0;
    nextStageButton.hidden = currentViewIndex === views.length - 1;
    emailMask.hidden = safeView !== "email";

    if (safeView === "pin") {
      guideText.textContent =
        "Coloca el PIN 071726 y presiona Continuar dentro de la herramienta.";
      nextStageButton.textContent = "Ya ingresé el PIN";
    } else if (safeView === "services") {
      guideText.textContent =
        "Selecciona Netflix dentro de la herramienta y continúa.";
      nextStageButton.textContent = "Ya seleccioné Netflix";
    } else {
      guideText.textContent =
        "Copia el correo, pégalo en el campo cubierto y pulsa Buscar código.";
    }
  }

  async function enterTool(data) {
    verified = data;
    authorizedEmail.textContent = data.email;
    maskedEmail.textContent = data.email;
    ownerLabel.textContent = [data.owner_name, data.parent_name]
      .filter(Boolean)
      .join(" / ") || "Sin nombre";
    countryLabel.textContent = data.country || "Sin configurar";
    ...
    await copyText(
      NETFLIX_GENERAL_PIN,
      "PIN 071726 copiado. Colócalo dentro de la herramienta."
    );
  ...
  async function validateProperty() {
    ...
    try {
      ...
      await enterTool(data);
    } catch (error) {
      verified = null;
      showValidationError(error.message || "No se pudo validar la cuenta.");
    } finally {
  ...
  copyEmailButton.onclick = () => copyText(
    verified?.email || "",
    "Correo copiado."
  );
  ...
  stageTabs.forEach(button => {
    button.onclick = () => setEmbedView(button.dataset.netflixEmbedView);
  });

  previousStageButton.onclick = () => {
    setEmbedView(views[Math.max(0, currentViewIndex - 1)]);
  };

  nextStageButton.onclick = () => {
    setEmbedView(views[Math.min(views.length - 1, currentViewIndex + 1)]);
  };
}
