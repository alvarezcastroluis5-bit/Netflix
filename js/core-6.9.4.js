...
export function openNetflixIntegrated() {
  ...
  const modal=openModal({
    ...
    extraWide:true,
    body:`
      <section class="netflix-native-shell">
        <div class="netflix-native-hero">
          <span class="netflix-native-logo">N</span>
          <div>
            <span class="eyebrow">ACCESO NETFLIX</span>
            <h3>Validación y solicitud</h3>
            <p>
              Sigue los cinco pasos para procesar la operación.
            </p>
          </div>
        </div>

        <div class="netflix-native-steps">
          <div class="netflix-native-step active" data-netflix-step="1">
            <span>1</span><strong>Correo</strong>
          </div>
          <div class="netflix-native-step" data-netflix-step="2">
            <span>2</span><strong>Propiedad</strong>
          </div>
          <div class="netflix-native-step" data-netflix-step="3">
            <span>3</span><strong>PIN</strong>
          </div>
          <div class="netflix-native-step" data-netflix-step="4">
            <span>4</span><strong>Operación</strong>
          </div>
          <div class="netflix-native-step" data-netflix-step="5">
            <span>5</span><strong>Procesar</strong>
          </div>
        </div>

        <form id="netflixNativeForm" class="netflix-native-form">
          <label>
          ...
          <div
            id="netflixNativeValidation"
            class="netflix-native-validation"
            hidden
          ></div>

          <div
            id="netflixPinSection"
            class="netflix-native-pin-section"
            hidden
          >
            <label>
              <span>3. Coloca el PIN de acceso</span>
              <div class="netflix-native-email-row">
                <input
                  name="pin"
                  type="password"
              ...
              <small>
                El PIN se comprueba de forma segura en Supabase.
              </small>
            </label>

            <div
              id="netflixPinValidation"
              class="netflix-native-validation"
              hidden
            ></div>
          </div>
          ...
          <fieldset
            id="netflixNativeActions"
            class="netflix-native-actions"
            disabled
          >
            <legend>4. Selecciona la operación</legend>
        ...
        <div
          id="netflixNativeResult"
          class="netflix-native-result"
          hidden
        ></div>
      </section>
    `,
    actions:`
      ...
      <button
        id="processNetflixNative"
        class="btn netflix-native-process"
        disabled
      >
  ...
  const form=$("#netflixNativeForm",modal.root);
  const email=form.elements.email;
  const pin=form.elements.pin;
  const validateButton=$("#validateNetflixNative",modal.root);
  const pinButton=$("#validateNetflixPin",modal.root);
  const validation=$("#netflixNativeValidation",modal.root);
  const pinSection=$("#netflixPinSection",modal.root);
  const pinValidation=$("#netflixPinValidation",modal.root);
  const actionsBox=$("#netflixNativeActions",modal.root);
  const processButton=$("#processNetflixNative",modal.root);
  const result=$("#netflixNativeResult",modal.root);
  const steps=$$("[data-netflix-step]",modal.root);
  ...
  function setStep(activeStep){
    steps.forEach(step=>{
      const number=Number(step.dataset.netflixStep);
  ...
  function clearOperation(){
    ...
    result.hidden=true;
    $$('input[name="action"]',modal.root).forEach(
  ...
  function clearPin(){
    pinVerified=false;
    pin.value="";
    pinValidation.hidden=true;
    pinValidation.innerHTML="";
    clearOperation();
  }
  ...
  email.addEventListener("input",()=>{
    if(
      verified
      &&email.value.trim().toLowerCase()!==verified.email.toLowerCase()
    ){
      clearValidation();
    }
  });
  ...
  async function validateProperty(){
    ...
    try{
      ...
      validation.innerHTML=`
        ...
        <span>
          Propietario: ${escapeHtml(data.owner_name||"Sin nombre")}
          ${data.parent_name
      ...
      validation.innerHTML=`
        <strong>No se pudo validar la propiedad</strong>
  ...
  async function validatePin(){
    ...
    pinButton.textContent="Validando...";

    try{
      const {data,error}=await supabase.rpc(
        "validate_netflix_pin_v30",
        {
      ...
      if(!data?.pin_validated){
        throw new Error("No se pudo validar el PIN.");
      }

      pinVerified=true;
      pinValidation.className="netflix-native-validation success";
      pinValidation.innerHTML=`
        <strong>✓ PIN validado correctamente</strong>
        <span>Ya puedes seleccionar la operación.</span>
      `;
      pinValidation.hidden=false;
      ...
      pinVerified=false;
      clearOperation();
      pinValidation.className="netflix-native-validation error";
      pinValidation.innerHTML=`
        <strong>PIN no válido</strong>
        <span>${escapeHtml(error.message)}</span>
      `;
      pinValidation.hidden=false;
      setStep(3);
      ...
      pinButton.textContent="Validar PIN";
    }
  }

  validateButton.onclick=validateProperty;
  pinButton.onclick=validatePin;
  ...
  processButton.onclick=async()=>{
    ...
    try{
      const {data,error}=await supabase.rpc(
        "create_netflix_action_request_v30",
        {
          p_email:verified.email,
          p_pin:pin.value,
          p_action:action
      ...
      result.className="netflix-native-result success";
      result.innerHTML=`
        <strong>✓ Solicitud procesada correctamente</strong>
        <p>${escapeHtml(data.message||"Solicitud enviada.")}</p>
        <small>
          ${escapeHtml(data.action_label||"Netflix")} ·
          ${escapeHtml(data.email||verified.email)}
        </small>
      `;
      result.hidden=false;
      steps.forEach(step=>step.classList.add("complete"));
      processButton.textContent="Solicitud procesada";
      toast("Solicitud Netflix procesada.");
    }catch(error){
      processButton.disabled=false;
      processButton.textContent="5. Procesar solicitud";
