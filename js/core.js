console.info("Centro Premium core V6.9.12 cargado");
...
export function openNetflixIntegrated() {
  ...
  const NETFLIX_EMBED_URL = "https://jp-streaming.pages.dev/";
  ...
  const modal = openModal({
    ...
    extraWide: true,
    body: `
      <section class="netflix-embed-shell">
        <section id="netflixValidationStage" class="netflix-validation-stage">
          <p class="netflix-flow-intro">
            Valida el correo, selecciona la operación y pulsa Continuar.
          </p>

          <form id="netflixEmbedForm" class="netflix-native-form">
            <label>
            ...
            <div
              id="netflixEmbedValidation"
              class="netflix-native-validation"
              hidden
            ></div>

            <fieldset
              id="netflixEmbedActions"
            ...
            <button
              id="continueNetflixEmbed"
              class="btn netflix-continue-button"
              type="button"
              hidden
            >
              Continuar
        ...
        <section
          id="netflixToolStage"
          class="netflix-tool-stage"
          hidden
        >
          <div class="netflix-tool-summary">
            <div class="netflix-tool-summary-text">
              <span class="eyebrow">CUENTA VALIDADA</span>
              <strong id="netflixToolEmail"></strong>
              <small>
                <span id="netflixToolOperation"></span>
                · País: <span id="netflixToolCountry"></span>
              </small>
            </div>

            <button
              id="returnNetflixValidation"
              class="btn secondary netflix-return-button"
              type="button"
            >
              Cambiar operación
            </button>
          ...
          <section class="netflix-tool-pin-card">
            <div>
              <span class="eyebrow">PIN DE ACCESO</span>
              <h3>${NETFLIX_GENERAL_PIN}</h3>
              <p>
                Copia el PIN, colócalo en la herramienta y presiona Continuar.
                Después selecciona Netflix, coloca nuevamente el correo validado
                y pulsa Buscar código.
              </p>
            </div>

            <div class="netflix-tool-copy-actions">
              <button
                id="copyNetflixEmbedPin"
          ...
          <div class="netflix-real-frame-shell">
            <div id="netflixFrameLoader" class="netflix-frame-loader">
  ...
  const form = $("#netflixEmbedForm", modal.root);
  const emailInput = form.elements.email;
  const validateButton = $("#validateNetflixEmbed", modal.root);
  const validationBox = $("#netflixEmbedValidation", modal.root);
  const actionsBox = $("#netflixEmbedActions", modal.root);
  const continueButton = $("#continueNetflixEmbed", modal.root);
  const validationStage = $("#netflixValidationStage", modal.root);
  const toolStage = $("#netflixToolStage", modal.root);
  const toolEmail = $("#netflixToolEmail", modal.root);
  const toolOperation = $("#netflixToolOperation", modal.root);
  const toolCountry = $("#netflixToolCountry", modal.root);
  const returnButton = $("#returnNetflixValidation", modal.root);
  const copyPinButton = $("#copyNetflixEmbedPin", modal.root);
  const copyEmailButton = $("#copyNetflixEmbedEmail", modal.root);
  const frame = $("#netflixRealFrame", modal.root);
  const frameLoader = $("#netflixFrameLoader", modal.root);
  ...
  continueButton.onclick = async () => {
    ...
    toolOperation.textContent = selectedActionLabel();
    ...
    if (frame.src === "about:blank" || !frame.src.includes("jp-streaming.pages.dev")) {
