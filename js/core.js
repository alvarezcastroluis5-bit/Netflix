...
export function openNetflixIntegrated(options = {}) {
  ...
  const verifiedCountry = String(options.country || "").trim();
  const requestId = String(options.requestId || "").trim();
  const vpnMessage = String(options.vpnMessage || "").trim();

  const jpUrl = "https://jp-streaming.pages.dev/";

  const modal = openModal({
    title:"Netflix · JP Streaming",
    extraWide:true,
    body:`
      <section class="jp-streaming-shell">
        <div class="jp-streaming-header">
          <div class="netflix-access-logo">N</div>

          <div class="jp-streaming-copy">
            <span class="eyebrow">SERVICIO NETFLIX</span>
            <h3>JP Streaming</h3>
            <p>
              ${verifiedEmail
                ?"La cuenta fue validada correctamente."
                :"Herramienta administrativa de Netflix."
              }
            </p>
          </div>

          <div class="jp-streaming-actions">
            <button
              id="reloadJpStreaming"
              class="btn secondary"
              type="button"
            >
              Recargar
        ...
        ${verifiedEmail?`
          ...
          <div class="vpn-country-notice">
            ...
            ${escapeHtml(verifiedCountry||"País sin configurar")}.
            ${escapeHtml(
              vpnMessage||
        ...
        <div class="jp-streaming-frame-wrapper">
          <div
            id="jpStreamingLoading"
            class="netflix-frame-loading"
          >
            <span class="netflix-loader"></span>
            <strong>Cargando JP Streaming…</strong>
          </div>

          <iframe
            id="jpStreamingFrame"
            class="jp-streaming-frame"
            src="${jpUrl}"
            title="JP Streaming"
        ...
        <div class="jp-streaming-notice">
          Si la herramienta no aparece dentro del panel, utiliza
          <strong>Abrir en otra pestaña</strong>.
        </div>
      </section>
    `
  });

  const iframe=$("#jpStreamingFrame",modal.root);
  const loading=$("#jpStreamingLoading",modal.root);
  const reloadButton=$("#reloadJpStreaming",modal.root);
  const externalButton=$("#openJpStreamingExternal",modal.root);
  const copyEmailButton=$("#copyVerifiedNetflixEmail",modal.root);
  ...
  reloadButton?.addEventListener("click",()=>{
    ...
    iframe.src=`${jpUrl}?refresh=${Date.now()}`;
  });
  ...
  externalButton?.addEventListener("click",()=>{
    ...
    if(!opened){
      toast(
        "El navegador bloqueó la pestaña. Habilita las ventanas emergentes.",
        "error"
      );
    }
  });
  ...
  copyEmailButton?.addEventListener("click",async()=>{
    ...
    window.setTimeout(()=>{
      if(copyEmailButton){
        copyEmailButton.textContent="Copiar correo";
      }
    },2200);
  });
}
