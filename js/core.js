console.info("Centro Premium core V6.9.21 cargado");
console.info("Centro Premium Netflix V6.9.21 cargado");
...
async function trackedFetch(input, init) {
  /*
   * V6.9.21
   * Las consultas automáticas, actualizaciones silenciosas,
   * notificaciones y refrescos internos no muestran el cargador global.
   *
   * El cargador solamente se muestra mediante showSection/withPageLoader,
   * es decir, cuando el usuario cambia manualmente de sección o ingresa.
   */
...
export function openNetflixIntegrated() {
      ...
      <section class="netflix-v16-shell">
        ...
        >
          ...
          >
            ...
            <div
              class="netflix-v21-continue-visual"
              aria-hidden="true"
            >
              Continuar
            </div>
            ...
            <div
              id="netflixV21ResultBackMask"
              class="netflix-v21-result-back-mask"
              aria-hidden="true"
              hidden
            ></div>

            <button
              id="netflixV21ResetAccount"
              class="netflix-v21-reset-account"
              type="button"
              hidden
            >
  ...
  const resultBackMask = $("#netflixV21ResultBackMask", modal.root);
  const resetAccountButton = $("#netflixV21ResetAccount", modal.root);
  ...
  function resetNetflixProcedure() {
    verified = null;
    selectedOperation = "";
    netflixFrameLoadCount = 0;
    ...
    toolEmail.textContent = "";
    toolCountry.textContent = "";
    toolOperation.textContent = "";
    coveredEmail.textContent = "";

    if (toolEmailCoverText) {
      toolEmailCoverText.textContent = "";
    }

    emailCover.classList.remove("show-email");

    if (resultBackMask) {
      resultBackMask.hidden = true;
    }

    if (resetAccountButton) {
      resetAccountButton.hidden = true;
    }
    ...
    toast("Puedes consultar otra cuenta.");
  }

  function showResetAccountControl() {
    if (resultBackMask) {
      resultBackMask.hidden = false;
    }

    if (resetAccountButton) {
      resetAccountButton.hidden = false;
    }
  }
  ...
  async function showTool(operation) {
    ...
    /*
     * El botón aparece después de que el distribuidor haya tenido
     * tiempo de colocar el PIN, elegir Netflix y buscar el código.
     * También aparece antes si el iframe realiza varias navegaciones.
     */
    resetButtonTimer = window.setTimeout(
      showResetAccountControl,
      12000
    );
  ...
  frame.addEventListener("load", () => {
    loader.hidden = true;
    netflixFrameLoadCount += 1;

    /*
     * Las páginas internas pueden navegar varias veces:
     * PIN, selección de servicio, búsqueda y resultado.
     */
    if (netflixFrameLoadCount >= 3) {
      showResetAccountControl();
    }
  });
  ...
  if (resetAccountButton) {
    resetAccountButton.onclick =
      resetNetflixProcedure;
  }
}
export async function showSection(section, onChange) {
  const requestedSection = document.getElementById(
    `section-${section}`
  );

  const alreadyActive = Boolean(
    requestedSection?.classList.contains("active")
  );
  ...
  /*
   * Solo muestra Cargando cuando el usuario cambia realmente
   * de una sección a otra. Un clic repetido en la sección activa
   * no vuelve a lanzar el cargador.
   */
  if (
    !alreadyActive &&
