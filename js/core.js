...
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, CREATE_USER_FUNCTION } from "./config.js";
...
export async function createUserSecurely(payload) {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/${CREATE_USER_FUNCTION}`,
      {
        method: "POST",
        headers: {
    ...
    try {
      ...
      result = { error: raw || "La función no devolvió una respuesta válida." };
    }
    ...
    if (!response.ok || result?.error) {
      throw new Error(
        result?.error ||
        result?.message ||
        `No se pudo crear el usuario. Código ${response.status}.`
      );
    }
    ...
    if (error?.name === "AbortError") {
      throw new Error(
        "La creación tardó demasiado. Revisa que la función hyper-processor esté desplegada."
      );
    }
    ...
    if (
      ...
      throw new Error(
        "No se pudo conectar con la función de usuarios. Actualiza hyper-processor con el archivo incluido."
      );
    }
