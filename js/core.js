const supabaseBrowserLibrary = globalThis.supabase;

if (!supabaseBrowserLibrary?.createClient) {
  const errorBox = document.getElementById("loginError");

  if (errorBox) {
    errorBox.hidden = false;
    errorBox.textContent =
      "No se pudo cargar la conexión con Supabase. Actualiza la página con Ctrl + F5.";
  }

  throw new Error("La librería de Supabase no se cargó.");
}

const { createClient } = supabaseBrowserLibrary;
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, USER_MANAGER_FUNCTION } from "./config.js?v=3.1";
