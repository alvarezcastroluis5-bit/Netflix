console.info("Centro Premium core V6.9.11 cargado");
...
export function openNetflixIntegrated() {
  ...
  const modal = openModal({
    ...
    extraWide: true,
    body: `
      <section class="netflix-clean-shell">
        <div class="netflix-clean-header-simple">
          <p>
            Valida el correo, elige la operación y trabaja dentro del mismo panel.
          </p>
        </div>

        <form id="netflixCleanForm" class="netflix-native-form">
          <label>
            <span>1. Correo de la cuenta Netflix</span>
            <div class="netflix-native-email-row">
              <input name="email" type="email" autocomplete="off" placeholder="cuenta@correo.com" required>
              <button id="validateNetflixClean" class="btn secondary" type="button">Validar propiedad</button>
            </div>
          </label>

          <div id="netflixCleanValidation" class="netflix-native-validation" hidden></div>

          <fieldset id="netflixCleanActions" class="netflix-native-actions" disabled>
            <legend>3. Selecciona la operación</legend>
        ...
        <section id="netflixCleanAccess" class="netflix-clean-access" hidden>
          <div class="netflix-clean-access-heading">
            <div>
              <span class="eyebrow">ACCESO AUTORIZADO</span>
              <h3>Datos para continuar</h3>
              <p>Usa el PIN general y el correo validado directamente aquí.</p>
            </div>
            <span class="netflix-clean-check">✓</span>
          </div>

          <div class="netflix-clean-copy-grid">
            <article class="netflix-clean-copy-card">
              <span>PIN general</span>
              <strong id="netflixCleanPin" class="netflix-clean-code">${NETFLIX_GENERAL_PIN}</strong>
              <button id="copyNetflixCleanPin" class="btn secondary" type="button">Copiar PIN</button>
            </article>

            <article class="netflix-clean-copy-card">
              <span>Correo validado</span>
              <strong id="netflixCleanEmail" class="netflix-clean-email"></strong>
              <button id="copyNetflixCleanEmail" class="btn secondary" type="button">Copiar correo</button>
            </article>
          </div>

          <div class="netflix-clean-operation-summary">
            <span>Operación seleccionada</span>
            <strong id="netflixCleanOperation"></strong>
          </div>

          <section class="netflix-inline-tool">
            <header class="netflix-inline-tool-header">
              <div>
                <span class="eyebrow">NETFLIX</span>
                <h3>Herramienta integrada</h3>
                <p>No se abre ninguna página externa. Todo se procesa dentro de este panel.</p>
              </div>
            </header>

            <div class="netflix-inline-tool-grid">
              <article class="netflix-inline-box">
                <span class="eyebrow">PASO 1</span>
                <h4>Coloca el PIN</h4>
                <p>El PIN general es el mismo para todas las operaciones.</p>
                <div class="netflix-inline-pin-row">
                  <input id="netflixInlinePin" class="netflix-inline-input netflix-inline-pin-input" type="text" inputmode="numeric" maxlength="6" placeholder="071726">
                  <button id="netflixInlineValidatePin" class="btn secondary" type="button">Validar PIN</button>
                </div>
                <small id="netflixInlinePinHelp" class="netflix-inline-help">PIN general: 071726</small>
              </article>

              <article class="netflix-inline-box" id="netflixInlineSearchBox" hidden>
                <span class="eyebrow">PASO 2</span>
                <h4>Netflix</h4>
                <p>La herramienta queda integrada aquí, sin logos externos ni redirecciones.</p>
                <label class="netflix-inline-label">
                  <span>Correo electrónico de Netflix</span>
                  <input id="netflixInlineEmail" class="netflix-inline-input" type="email" autocomplete="off" placeholder="cuenta@correo.com">
                </label>
                <div class="netflix-inline-search-actions">
                  <button id="copyNetflixInlinePin" class="btn secondary" type="button">Copiar PIN</button>
                  <button id="copyNetflixInlineEmail" class="btn secondary" type="button">Copiar correo</button>
                  <button id="searchNetflixInlineCode" class="btn netflix-clean-open" type="button">Buscar código</button>
                </div>
              </article>
            </div>
          </section>
  ...
  const form = $("#netflixCleanForm", modal.root);
  const email = form.elements.email;
  const validateButton = $("#validateNetflixClean", modal.root);
  const validation = $("#netflixCleanValidation", modal.root);
  const actionsBox = $("#netflixCleanActions", modal.root);
  const accessSection = $("#netflixCleanAccess", modal.root);
  const accessEmail = $("#netflixCleanEmail", modal.root);
  const accessOperation = $("#netflixCleanOperation", modal.root);
  const copyPinButton = $("#copyNetflixCleanPin", modal.root);
  const copyEmailButton = $("#copyNetflixCleanEmail", modal.root);
  const inlinePinInput = $("#netflixInlinePin", modal.root);
  const inlineValidatePinButton = $("#netflixInlineValidatePin", modal.root);
  const inlinePinHelp = $("#netflixInlinePinHelp", modal.root);
  const inlineSearchBox = $("#netflixInlineSearchBox", modal.root);
  const inlineEmailInput = $("#netflixInlineEmail", modal.root);
  const inlineCopyPinButton = $("#copyNetflixInlinePin", modal.root);
  const inlineCopyEmailButton = $("#copyNetflixInlineEmail", modal.root);
  const inlineSearchButton = $("#searchNetflixInlineCode", modal.root);
  ...
  function selectedActionLabel() {
    ...
    return actions.find(([action]) => action === value)?.[2] || "";
  }

  function clearInlineTool() {
    if (inlinePinInput) inlinePinInput.value = "";
    if (inlineEmailInput) inlineEmailInput.value = "";
    if (inlineSearchBox) inlineSearchBox.hidden = true;
    if (inlinePinHelp) {
      inlinePinHelp.textContent = "PIN general: 071726";
      inlinePinHelp.className = "netflix-inline-help";
    }
  }

  function clearAccess() {
    accessSection.hidden = true;
    accessEmail.textContent = "";
    accessOperation.textContent = "";
    clearInlineTool();
  }

  function clearOperation() {
    actionsBox.disabled = true;
    $$("input[name='action']", modal.root).forEach(input => { input.checked = false; });
    clearAccess();
  }
  ...
  email.addEventListener("input", () => {
    if (verified && email.value.trim().toLowerCase() !== verified.email.toLowerCase()) {
      clearValidation();
    }
  });
  ...
  async function validateProperty() {
    ...
    try {
      const { data, error } = await supabase.rpc("validate_netflix_access_v29", { p_email: email.value.trim().toLowerCase() });
      if (error) throw error;
      verified = data;
      email.value = data.email;
      validation.className = "netflix-native-validation success";
      validation.innerHTML = `
        <strong>✓ 2. Propiedad validada correctamente</strong>
        <span>Propietario: ${escapeHtml(data.owner_name || "Sin nombre")}${data.parent_name ? ` / ${escapeHtml(data.parent_name)}` : ""}</span>
      ...
      clearValidation();
      validation.className = "netflix-native-validation error";
      validation.innerHTML = `<strong>No se pudo validar la propiedad</strong><span>${escapeHtml(error.message)}</span>`;
      validation.hidden = false;
    } finally {
  ...
  $$("input[name='action']", modal.root).forEach(input => {
    input.onchange = () => {
      ...
      accessEmail.textContent = verified.email;
      accessOperation.textContent = selectedActionLabel();
      accessSection.hidden = false;
      clearInlineTool();
      accessSection.scrollIntoView({ behavior: "smooth", block: "nearest" });
    };
  });

  copyPinButton.onclick = () => copyText(NETFLIX_GENERAL_PIN, "PIN copiado.");
  copyEmailButton.onclick = () => copyText(verified?.email || "", "Correo copiado.");
  inlineCopyPinButton.onclick = () => copyText(NETFLIX_GENERAL_PIN, "PIN copiado.");
  inlineCopyEmailButton.onclick = () => copyText(verified?.email || inlineEmailInput?.value || "", "Correo copiado.");

  inlineValidatePinButton.onclick = async () => {
    const enteredPin = String(inlinePinInput?.value || "").trim();
    if (enteredPin !== NETFLIX_GENERAL_PIN) {
      inlineSearchBox.hidden = true;
      inlinePinHelp.textContent = "PIN incorrecto. Usa el PIN general 071726.";
      inlinePinHelp.className = "netflix-inline-help error";
      toast("El PIN no es válido.", "error");
      return;
    }
    inlinePinHelp.textContent = "PIN validado correctamente.";
    inlinePinHelp.className = "netflix-inline-help success";
    inlineEmailInput.value = verified?.email || "";
    inlineSearchBox.hidden = false;
    inlineSearchBox.scrollIntoView({ behavior: "smooth", block: "nearest" });
    toast("PIN validado.");
  };

  inlineSearchButton.onclick = async () => {
    if (!verified) {
      await validateProperty();
      return;
    }
    if (!selectedAction()) {
      toast("Selecciona la operación.", "error");
      return;
    }
    const enteredPin = String(inlinePinInput?.value || "").trim();
    if (enteredPin !== NETFLIX_GENERAL_PIN) {
      toast("Primero valida el PIN 071726.", "error");
      return;
    }
    const finalEmail = String(inlineEmailInput?.value || "").trim().toLowerCase();
    if (!finalEmail) {
      inlineEmailInput.reportValidity();
      toast("Coloca el correo de Netflix.", "error");
      return;
    }
    if (verified?.email && finalEmail !== verified.email.toLowerCase()) {
      toast("El correo debe coincidir con el validado.", "error");
      return;
    }
    toast(`Solicitud preparada: ${selectedActionLabel()} para ${verified.email}.`);
  };
}
