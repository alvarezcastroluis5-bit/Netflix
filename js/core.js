...
export function openNetflixIntegrated() {
      ...
      <section class="netflix-cropped-shell">
        <div class="netflix-access-card">
          ...
          <div class="netflix-access-copy">
            ...
            <h3>PIN de acceso</h3>
            <p>
              Copia el PIN, colócalo en la ventana inferior y sigue los tres pasos.
            </p>
          </div>
        ...
        <div class="netflix-view-steps" role="tablist" aria-label="Pasos de Netflix">
          <button class="netflix-step active" data-netflix-view="pin" type="button">
            <span>1</span>
            <div><strong>Ingresar PIN</strong><small>Escribe 071726</small></div>
          </button>

          <button class="netflix-step" data-netflix-view="service" type="button">
            <span>2</span>
            <div><strong>Seleccionar Netflix</strong><small>Oculta el encabezado</small></div>
          </button>

          <button class="netflix-step" data-netflix-view="code" type="button">
            <span>3</span>
            <div><strong>Buscar código</strong><small>Formulario final</small></div>
          </button>
        </div>
        ...
        <div class="netflix-crop-actions">
          <div id="netflixViewInstruction">
            <strong>Paso 1:</strong> pega el PIN y presiona Continuar.
          </div>
  ...
  const steps = $$("[data-netflix-view]", modal.root);

  const instructions = {
    pin: "<strong>Paso 1:</strong> pega el PIN y presiona Continuar.",
    service: "<strong>Paso 2:</strong> selecciona Netflix, presiona Continuar y luego abre el paso 3.",
    code: "<strong>Paso 3:</strong> ingresa el correo de Netflix y busca el código."
  };

  const switchView = view => {
    viewport.dataset.view = view;
    steps.forEach(step => {
      step.classList.toggle("active", step.dataset.netflixView === view);
    });
    instruction.innerHTML = instructions[view] || instructions.pin;
  };

  steps.forEach(step => {
    step.addEventListener("click", () => switchView(step.dataset.netflixView));
  });
